function enableThemeToggle() {
  ...
  function computeHlHref(theme) {
    if (!hlLink || !hlLink.href) return null;
    const url = new URL(hlLink.href, window.location.href);
    url.pathname = url.pathname.replace(/hl-(dark|light)\.css$/i, `hl-${theme}.css`);
    return url.toString();
  }
  ...
  function applyTheme(theme) {
    ...
    const newHref = computeHlHref(theme);
    if (hlLink && newHref) hlLink.href = newHref;
  ...
  function toggleTheme() {
    ...
    console.log('Toggling from', current, 'to', next); // Debug log
    applyTheme(next);
  }
  ...
  const setupToggleHandler = (selector) => {
    const btn = document.querySelector(selector);
    if (btn) {
      console.log('Setting up handler for', selector); // Debug log
      btn.addEventListener('click', (e) => {
        e.preventDefault();
        e.stopPropagation();
        console.log('Theme toggle clicked:', selector); // Debug log
        toggleTheme();
      });
    }
  };

  // Setup handlers for both buttons
  setupToggleHandler('#theme-toggle');
  setupToggleHandler('#navbar-theme-toggle');
...
function addCopyBtns() {
  ...
  document.querySelectorAll('pre').forEach(block => {
    ...
    if (block.closest('.codeblock')) return; // already wrapped
    if (block.dataset.copyInjected === '1') return; // processed
