...
Using mmap and `string_view` is one thing - it will definitely reduce the memory-related lags, but the CPU-bound side of things need to be allievated too, which requires a good parser (in place of getline, and `std::basic_string<char>::find()`). In order to parse a single line, I wanted to benchmark the differences between 3 implementations - one with `memchr`, one with a simple hand-rolled loop, and another one with SIMD. For the SIMD and hand-rolled ones, I will also have variants that do a "rolling" hash.

- In a list of strings (city names;measurement), iterate through all strings
- For each string, find the city name and measurement
- For the city name, calculate the hash

Here are the 3 respective benchmark functions I wrote for the same:
 -->
